export const RU_TRANSLATIONS: Record<string, string[]> = {
  // Smileys
  smile: ['улыбка','улыбаться'],
  happy: ['счастливый','радостный','весёлый'],
  laugh: ['смех','смеяться','хаха'],
  cry: ['плакать','плач','слеза'],
  tears: ['слёзы'],
  sad: ['грустный','печальный','тоска'],
  angry: ['злой','сердитый','ярость'],
  mad: ['бешеный','злой'],
  love: ['любовь','люблю','сердце'],
  kiss: ['поцелуй','целовать'],
  heart: ['сердце'],
  wink: ['подмигивание','подмигнуть'],
  shy: ['стеснительный','застенчивый'],
  think: ['думать','размышлять'],
  sleep: ['спать','сон','спящий'],
  sick: ['больной','болезнь'],
  cool: ['крутой','классный'],
  crazy: ['сумасшедший','безумный'],
  scared: ['испуганный','страх','боязнь'],
  surprise: ['сюрприз','удивление'],
  shock: ['шок','потрясение'],
  devil: ['дьявол','чёрт'],
  evil: ['злой','злодей'],
  ghost: ['привидение','призрак'],
  skull: ['череп','смерть'],
  robot: ['робот'],
  clown: ['клоун'],
  poop: ['какашка','фекалии'],
  party: ['вечеринка','праздник'],
  nerd: ['ботаник','заучка'],
  // People
  hand: ['рука','ладонь'],
  wave: ['волна','помахать','привет'],
  hello: ['привет','здравствуйте'],
  bye: ['пока','до свидания'],
  stop: ['стоп','остановить'],
  point: ['указать','показать'],
  finger: ['палец'],
  thumb: ['большой палец'],
  thumbs: ['большие пальцы'],
  fist: ['кулак'],
  punch: ['удар','ударить'],
  strong: ['сильный','сила'],
  muscle: ['мускул','мышца','бицепс'],
  clap: ['хлопать','аплодисменты'],
  pray: ['молиться','пожалуйста'],
  write: ['писать','написать'],
  ear: ['ухо'],
  nose: ['нос'],
  eye: ['глаз'],
  eyes: ['глаза'],
  tongue: ['язык'],
  mouth: ['рот','губы'],
  foot: ['стопа','нога'],
  leg: ['нога'],
  ok: ['ок','хорошо','ладно'],
  victory: ['победа'],
  left: ['влево','лево'],
  right: ['вправо','право'],
  up: ['вверх'],
  down: ['вниз'],
  baby: ['младенец','малыш','ребёнок'],
  boy: ['мальчик','парень'],
  girl: ['девочка','девушка'],
  man: ['мужчина','мужик'],
  woman: ['женщина'],
  old: ['старый','старик','пожилой'],
  police: ['полиция','полицейский'],
  ninja: ['ниндзя'],
  prince: ['принц'],
  princess: ['принцесса'],
  angel: ['ангел'],
  santa: ['дед мороз','санта'],
  hero: ['герой','супергерой'],
  facepalm: ['рукалицо','фейспалм'],
  shrug: ['не знаю','пожимание плечами'],
  // Animals
  dog: ['собака','пёс'],
  puppy: ['щенок'],
  cat: ['кот','кошка','котик'],
  mouse: ['мышь','мышка'],
  rabbit: ['кролик','заяц'],
  bunny: ['зайчик','кролик'],
  fox: ['лиса','лисица'],
  bear: ['медведь'],
  panda: ['панда'],
  lion: ['лев'],
  tiger: ['тигр'],
  monkey: ['обезьяна'],
  horse: ['лошадь','конь'],
  cow: ['корова'],
  pig: ['свинья','поросёнок'],
  chicken: ['курица','цыплёнок'],
  bird: ['птица','птичка'],
  fish: ['рыба'],
  whale: ['кит'],
  dolphin: ['дельфин'],
  shark: ['акула'],
  snake: ['змея'],
  butterfly: ['бабочка'],
  bee: ['пчела'],
  spider: ['паук'],
  turtle: ['черепаха'],
  frog: ['лягушка','жаба'],
  dragon: ['дракон'],
  unicorn: ['единорог'],
  flower: ['цветок','цветы'],
  rose: ['роза'],
  tree: ['дерево'],
  // Food
  apple: ['яблоко'],
  banana: ['банан'],
  orange: ['апельсин','оранжевый'],
  lemon: ['лимон'],
  grape: ['виноград'],
  watermelon: ['арбуз'],
  strawberry: ['клубника','земляника'],
  peach: ['персик'],
  mango: ['манго'],
  pineapple: ['ананас'],
  tomato: ['помидор','томат'],
  avocado: ['авокадо'],
  corn: ['кукуруза'],
  carrot: ['морковь','морковка'],
  potato: ['картошка','картофель'],
  bread: ['хлеб','батон'],
  cheese: ['сыр'],
  meat: ['мясо'],
  burger: ['бургер','гамбургер'],
  pizza: ['пицца'],
  taco: ['тако'],
  burrito: ['буррито'],
  egg: ['яйцо'],
  rice: ['рис'],
  pasta: ['паста','макароны'],
  sushi: ['суши','роллы'],
  cake: ['торт','пирог'],
  cookie: ['печенье'],
  candy: ['конфета','сладость'],
  chocolate: ['шоколад'],
  coffee: ['кофе'],
  tea: ['чай'],
  beer: ['пиво'],
  wine: ['вино'],
  water: ['вода'],
  ice: ['лёд','мороженое'],
  // Travel
  car: ['машина','автомобиль'],
  taxi: ['такси'],
  bus: ['автобус'],
  truck: ['грузовик','фура'],
  bike: ['велосипед'],
  motorcycle: ['мотоцикл','мото'],
  airplane: ['самолёт'],
  plane: ['самолёт'],
  rocket: ['ракета'],
  boat: ['лодка','катер'],
  ship: ['корабль','судно'],
  train: ['поезд','электричка'],
  house: ['дом','жильё'],
  school: ['школа'],
  hospital: ['больница'],
  church: ['церковь','храм'],
  mountain: ['гора','горы'],
  beach: ['пляж'],
  island: ['остров'],
  sun: ['солнце'],
  moon: ['луна','месяц'],
  star: ['звезда'],
  rain: ['дождь'],
  snow: ['снег'],
  fire: ['огонь','пламя'],
  rainbow: ['радуга'],
  ocean: ['океан','море'],
  // Activities
  soccer: ['футбол'],
  basketball: ['баскетбол'],
  tennis: ['теннис'],
  swim: ['плавать','плавание'],
  sport: ['спорт'],
  gym: ['спортзал','тренажёрка'],
  yoga: ['йога'],
  surf: ['сёрфинг'],
  climb: ['лазать','скалолазание'],
  run: ['бегать','бег'],
  game: ['игра','видеоигра'],
  music: ['музыка'],
  guitar: ['гитара'],
  piano: ['пианино','рояль'],
  sing: ['петь','пение'],
  movie: ['фильм','кино'],
  art: ['искусство'],
  paint: ['рисовать','краска'],
  dance: ['танец','танцевать'],
  trophy: ['трофей','кубок'],
  medal: ['медаль'],
  celebrate: ['праздновать','отмечать'],
  birthday: ['день рождения'],
  gift: ['подарок'],
  christmas: ['рождество','новый год'],
  halloween: ['хэллоуин'],
  // Objects
  phone: ['телефон','смартфон','мобильный'],
  computer: ['компьютер','комп'],
  laptop: ['ноутбук'],
  camera: ['камера','фотоаппарат'],
  tv: ['телевизор','тв'],
  money: ['деньги','бабки'],
  key: ['ключ'],
  lock: ['замок','закрыть'],
  tool: ['инструмент'],
  book: ['книга'],
  email: ['почта','электронная почта'],
  trash: ['мусор','урна'],
  pill: ['таблетка','пилюля'],
  bomb: ['бомба'],
  crown: ['корона'],
  ring: ['кольцо'],
  glasses: ['очки'],
  shoe: ['ботинок','обувь','туфля'],
  hat: ['шляпа','шапка'],
  scissors: ['ножницы'],
  pencil: ['карандаш'],
  pen: ['ручка'],
  folder: ['папка'],
  package: ['посылка','пакет'],
  cart: ['тележка','корзина'],
  shield: ['щит'],
  sword: ['меч'],
  swords: ['мечи'],
  teddy: ['плюшевый мишка','мишка'],
  dress: ['платье'],
  backpack: ['рюкзак'],
  lipstick: ['помада'],
  science: ['наука'],
  dna: ['днк','генетика'],
  // More food
  soup: ['суп','бульон'],
  salad: ['салат'],
  noodles: ['лапша'],
  shrimp: ['креветка'],
  donut: ['пончик'],
  popcorn: ['попкорн'],
  butter: ['масло','сливочное масло'],
  salt: ['соль'],
  spicy: ['острый'],
  sweet: ['сладкий'],
  delicious: ['вкусный','вкуснятина'],
  hungry: ['голодный','голод'],
  drink: ['напиток','пить'],
  cocktail: ['коктейль'],
  champagne: ['шампанское'],
  juice: ['сок'],
  // More animals
  elephant: ['слон'],
  giraffe: ['жираф'],
  zebra: ['зебра'],
  gorilla: ['горилла'],
  camel: ['верблюд'],
  kangaroo: ['кенгуру'],
  deer: ['олень'],
  owl: ['сова','филин'],
  eagle: ['орёл'],
  duck: ['утка'],
  penguin: ['пингвин'],
  parrot: ['попугай'],
  flamingo: ['фламинго'],
  swan: ['лебедь'],
  wolf: ['волк'],
  bat: ['летучая мышь'],
  scorpion: ['скорпион'],
  snail: ['улитка'],
  ant: ['муравей'],
  mosquito: ['комар'],
  worm: ['червь','червяк'],
  octopus: ['осьминог'],
  squid: ['кальмар'],
  crocodile: ['крокодил'],
  lizard: ['ящерица'],
  hedgehog: ['ёжик','ёж'],
  otter: ['выдра'],
  sloth: ['ленивец'],
  beaver: ['бобр','бобёр'],
  llama: ['лама','альпака'],
  goat: ['козёл','коза'],
  sheep: ['овца','баран'],
  hippo: ['бегемот'],
  rhino: ['носорог'],
  cactus: ['кактус'],
  clover: ['клевер'],
  leaf: ['лист','листва'],
  plant: ['растение'],
  herb: ['трава','зелень'],
  // More travel
  volcano: ['вулкан'],
  globe: ['глобус','мир','планета'],
  weather: ['погода'],
  storm: ['шторм','буря'],
  thunder: ['гром','молния'],
  wind: ['ветер'],
  cold: ['холод','холодный'],
  hot: ['жара','горячий'],
  cloud: ['облако','туча'],
  // More activities
  boxing: ['бокс'],
  karate: ['каратэ'],
  archery: ['стрельба из лука'],
  fishing: ['рыбалка'],
  bowling: ['боулинг'],
  chess: ['шахматы'],
  dice: ['кости','кубик'],
  target: ['мишень','цель'],
  dart: ['дротик'],
  casino: ['казино'],
  fireworks: ['фейерверк','салют'],
  balloon: ['воздушный шар','шарик'],
  // More people
  child: ['ребёнок','дитя'],
  adult: ['взрослый'],
  elder: ['пожилой','старец'],
  beard: ['борода'],
  blonde: ['блондин','блондинка'],
  guard: ['охранник','стража'],
  construction: ['строительство','строитель'],
  royal: ['королевский'],
  king: ['король'],
  queen: ['королева'],
  mage: ['маг','чародей'],
  wizard: ['волшебник','колдун'],
  fairy: ['фея'],
  mermaid: ['русалка'],
  vampire: ['вампир'],
  magic: ['магия','волшебство'],
  deaf: ['глухой'],
  bow: ['поклон'],
  // Symbols
  check: ['галочка','готово','да'],
  no: ['нет','запрет'],
  warning: ['предупреждение','внимание','опасность'],
  question: ['вопрос'],
  exclamation: ['восклицание'],
  infinity: ['бесконечность'],
  recycle: ['переработка','вторсырьё'],
  play: ['воспроизвести','играть'],
  pause: ['пауза'],
  shuffle: ['перемешать'],
  repeat: ['повторить'],
  plus: ['плюс','прибавить'],
  minus: ['минус','отнять'],
  divide: ['разделить','деление'],
  multiply: ['умножить','умножение'],
  copyright: ['авторское право'],
  trademark: ['торговая марка'],
  note: ['нота','заметка'],
  notes: ['ноты','заметки'],
  red: ['красный'],
  blue: ['синий','голубой'],
  green: ['зелёный'],
  yellow: ['жёлтый'],
  purple: ['фиолетовый','сиреневый'],
  black: ['чёрный'],
  white: ['белый'],
  brown: ['коричневый'],
  pink: ['розовый'],
  grey: ['серый'],
  // Professions
  doctor: ['доктор','врач'],
  teacher: ['учитель','учительница','преподаватель'],
  student: ['студент','ученик'],
  judge: ['судья'],
  farmer: ['фермер','крестьянин'],
  chef: ['шеф-повар','повар'],
  cook: ['готовить','повар'],
  mechanic: ['механик'],
  factory: ['завод','фабрика'],
  worker: ['рабочий','работник'],
  office: ['офис'],
  scientist: ['учёный'],
  programmer: ['программист'],
  developer: ['разработчик'],
  coder: ['кодер','программист'],
  singer: ['певец','певица'],
  artist: ['артист','художник'],
  painter: ['художник','маляр'],
  pilot: ['пилот','лётчик'],
  astronaut: ['космонавт','астронавт'],
  firefighter: ['пожарный'],
  // Family
  family: ['семья'],
  parents: ['родители'],
  couple: ['пара'],
  son: ['сын'],
  daughter: ['дочь','дочка'],
  children: ['дети'],
  pregnant: ['беременная'],
  // Person actions
  walk: ['ходить','идти','прогулка'],
  jog: ['бег трусцой','пробежка'],
  kneel: ['колени','стоять на коленях'],
  stand: ['стоять'],
  // More animals 2
  jellyfish: ['медуза'],
  seal: ['тюлень','нерпа'],
  coral: ['коралл','риф'],
  beetle: ['жук'],
  cockroach: ['таракан'],
  cricket: ['сверчок'],
  dove: ['голубь','мир'],
  skunk: ['скунс'],
  badger: ['барсук'],
  mammoth: ['мамонт'],
  bison: ['бизон','зубр'],
  feather: ['перо'],
  paw: ['лапа','лапка'],
  palm: ['пальма'],
  seedling: ['росток','саженец'],
  lotus: ['лотос'],
  tulip: ['тюльпан'],
  bouquet: ['букет'],
  // More food 2
  waffle: ['вафля'],
  pretzel: ['крендель','претцель'],
  bagel: ['бублик','бейгл'],
  dumpling: ['пельмень','вареник'],
  mate: ['мате'],
  sake: ['саке'],
  ginger: ['имбирь'],
  olive: ['оливка','маслина'],
  beans: ['бобы','фасоль'],
  // Signs
  atm: ['банкомат'],
  restroom: ['туалет','уборная'],
  wheelchair: ['инвалидная коляска'],
  parking: ['парковка','стоянка'],
  peace: ['мир'],
};
