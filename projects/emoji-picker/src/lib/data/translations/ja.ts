export const JA_TRANSLATIONS: Record<string, string[]> = {
  // Smileys
  smile: ['笑顔', 'にこにこ'],
  happy: ['嬉しい', '幸せ', '楽しい'],
  laugh: ['笑う', '爆笑', 'わらう'],
  cry: ['泣く', '涙', 'なく'],
  tears: ['涙', 'なみだ'],
  sad: ['悲しい', 'かなしい'],
  angry: ['怒り', '怒る', 'おこる'],
  mad: ['激怒', 'ぶちぎれ'],
  love: ['愛', '大好き', 'ラブ'],
  kiss: ['キス', '口づけ', 'チュー'],
  heart: ['ハート', '心', '愛'],
  wink: ['ウィンク', '片目'],
  shy: ['恥ずかしい', '照れる', 'てれる'],
  think: ['考える', '思考', 'かんがえる'],
  sleep: ['寝る', '眠い', 'ねむい'],
  sick: ['病気', '具合悪い', 'びょうき'],
  cool: ['かっこいい', 'クール'],
  crazy: ['狂う', 'クレイジー', 'やばい'],
  scared: ['怖い', 'こわい', '恐怖'],
  surprise: ['驚き', 'びっくり', 'おどろき'],
  shock: ['ショック', '衝撃'],
  devil: ['悪魔', 'デビル', 'あくま'],
  evil: ['邪悪', '悪い'],
  ghost: ['お化け', '幽霊', 'おばけ'],
  skull: ['骸骨', 'ドクロ', 'がいこつ'],
  robot: ['ロボット'],
  clown: ['ピエロ', '道化師'],
  poop: ['うんち', 'うんこ'],
  party: ['パーティー', 'お祝い'],
  nerd: ['オタク', 'ナード'],

  // People
  hand: ['手', 'て'],
  wave: ['手を振る', 'バイバイ'],
  hello: ['こんにちは', 'やあ', 'ハロー'],
  bye: ['バイバイ', 'さようなら'],
  stop: ['止まれ', 'ストップ'],
  point: ['指さす', 'ポイント'],
  finger: ['指', 'ゆび'],
  thumb: ['親指', 'おやゆび'],
  thumbs: ['いいね', 'グッド'],
  fist: ['拳', 'こぶし', 'グー'],
  punch: ['パンチ', '殴る'],
  strong: ['強い', 'つよい'],
  muscle: ['筋肉', 'きんにく', 'マッチョ'],
  clap: ['拍手', 'はくしゅ', 'パチパチ'],
  pray: ['お祈り', '祈る', 'いのる'],
  write: ['書く', 'かく'],
  ear: ['耳', 'みみ'],
  nose: ['鼻', 'はな'],
  eye: ['目', 'め'],
  eyes: ['目', '両目'],
  tongue: ['舌', 'べろ', 'した'],
  mouth: ['口', 'くち'],
  foot: ['足', 'あし'],
  leg: ['脚', 'あし'],
  ok: ['オーケー', 'OK', '大丈夫'],
  victory: ['勝利', 'ピース'],
  left: ['左', 'ひだり'],
  right: ['右', 'みぎ'],
  up: ['上', 'うえ'],
  down: ['下', 'した'],
  baby: ['赤ちゃん', 'ベビー'],
  boy: ['男の子', '少年'],
  girl: ['女の子', '少女'],
  man: ['男', '男性', 'おとこ'],
  woman: ['女', '女性', 'おんな'],
  old: ['老人', 'お年寄り'],
  police: ['警察', 'おまわりさん'],
  ninja: ['忍者', 'にんじゃ'],
  prince: ['王子', 'おうじ'],
  princess: ['お姫様', '姫', 'プリンセス'],
  angel: ['天使', 'エンジェル'],
  santa: ['サンタ', 'サンタクロース'],
  hero: ['ヒーロー', '英雄'],
  facepalm: ['顔を覆う', 'がっかり'],
  shrug: ['肩をすくめる', 'わからない'],

  // Animals
  dog: ['犬', 'いぬ', 'ワンちゃん'],
  puppy: ['子犬', 'こいぬ'],
  cat: ['猫', 'ねこ', 'にゃんこ'],
  mouse: ['ねずみ', 'マウス'],
  rabbit: ['うさぎ', 'ウサギ'],
  bunny: ['うさぎ', 'バニー'],
  fox: ['きつね', 'キツネ'],
  bear: ['くま', 'クマ'],
  panda: ['パンダ'],
  lion: ['ライオン', '獅子'],
  tiger: ['虎', 'トラ', 'タイガー'],
  monkey: ['猿', 'さる', 'サル'],
  horse: ['馬', 'うま'],
  cow: ['牛', 'うし'],
  pig: ['豚', 'ぶた'],
  chicken: ['鶏', 'にわとり', 'チキン'],
  bird: ['鳥', 'とり'],
  fish: ['魚', 'さかな'],
  whale: ['くじら', 'クジラ'],
  dolphin: ['イルカ', 'いるか'],
  shark: ['サメ', 'さめ'],
  snake: ['蛇', 'へび'],
  butterfly: ['蝶', 'ちょうちょ'],
  bee: ['蜂', 'ハチ', 'みつばち'],
  spider: ['蜘蛛', 'クモ', 'くも'],
  turtle: ['亀', 'カメ', 'かめ'],
  frog: ['蛙', 'カエル', 'かえる'],
  dragon: ['龍', 'ドラゴン', 'りゅう'],
  unicorn: ['ユニコーン', '一角獣'],
  flower: ['花', 'はな'],
  rose: ['バラ', '薔薇'],
  tree: ['木', '樹木', 'き'],

  // Food
  apple: ['りんご', 'リンゴ'],
  banana: ['バナナ'],
  orange: ['オレンジ', 'みかん'],
  lemon: ['レモン'],
  grape: ['ぶどう', 'ブドウ'],
  watermelon: ['スイカ', 'すいか'],
  strawberry: ['いちご', 'イチゴ'],
  peach: ['桃', 'もも'],
  mango: ['マンゴー'],
  pineapple: ['パイナップル'],
  tomato: ['トマト'],
  avocado: ['アボカド'],
  corn: ['トウモロコシ', 'とうもろこし'],
  carrot: ['にんじん', 'ニンジン'],
  potato: ['じゃがいも', 'ポテト'],
  bread: ['パン', 'ぱん'],
  cheese: ['チーズ'],
  meat: ['肉', 'にく'],
  burger: ['ハンバーガー', 'バーガー'],
  pizza: ['ピザ'],
  taco: ['タコス'],
  burrito: ['ブリトー'],
  egg: ['卵', 'たまご'],
  rice: ['ご飯', 'ごはん', '米'],
  pasta: ['パスタ'],
  sushi: ['寿司', 'すし'],
  cake: ['ケーキ'],
  cookie: ['クッキー'],
  candy: ['飴', 'キャンディ', 'あめ'],
  chocolate: ['チョコレート', 'チョコ'],
  coffee: ['コーヒー', '珈琲'],
  tea: ['お茶', 'おちゃ', '紅茶'],
  beer: ['ビール'],
  wine: ['ワイン'],
  water: ['水', 'みず'],
  ice: ['氷', 'こおり', 'アイス'],

  // Travel
  car: ['車', 'くるま', '自動車'],
  taxi: ['タクシー'],
  bus: ['バス'],
  truck: ['トラック'],
  bike: ['自転車', 'じてんしゃ'],
  motorcycle: ['バイク', 'オートバイ'],
  airplane: ['飛行機', 'ひこうき'],
  plane: ['飛行機', 'ひこうき'],
  rocket: ['ロケット'],
  boat: ['ボート', '船'],
  ship: ['船', 'ふね'],
  train: ['電車', 'でんしゃ'],
  house: ['家', 'いえ', 'うち'],
  school: ['学校', 'がっこう'],
  hospital: ['病院', 'びょういん'],
  church: ['教会', 'きょうかい'],
  mountain: ['山', 'やま'],
  beach: ['ビーチ', '海岸', '砂浜'],
  island: ['島', 'しま'],
  sun: ['太陽', 'たいよう', '日'],
  moon: ['月', 'つき'],
  star: ['星', 'ほし', 'スター'],
  rain: ['雨', 'あめ'],
  snow: ['雪', 'ゆき'],
  fire: ['火', '炎', 'ひ'],
  rainbow: ['虹', 'にじ'],
  ocean: ['海', 'うみ', '大洋'],

  // Activities
  soccer: ['サッカー'],
  basketball: ['バスケ', 'バスケットボール'],
  tennis: ['テニス'],
  swim: ['水泳', 'すいえい', '泳ぐ'],
  sport: ['スポーツ', '運動'],
  gym: ['ジム', '体育館'],
  yoga: ['ヨガ'],
  surf: ['サーフィン'],
  climb: ['登る', 'クライミング', 'のぼる'],
  run: ['走る', 'ランニング', 'はしる'],
  game: ['ゲーム', '試合'],
  music: ['音楽', 'おんがく', 'ミュージック'],
  guitar: ['ギター'],
  piano: ['ピアノ'],
  sing: ['歌う', 'うたう', '歌'],
  movie: ['映画', 'えいが', 'ムービー'],
  art: ['アート', '芸術', '美術'],
  paint: ['絵の具', 'ペイント', '塗る'],
  dance: ['ダンス', '踊る', 'おどる'],
  trophy: ['トロフィー', '優勝'],
  medal: ['メダル'],
  celebrate: ['お祝い', '祝う'],
  birthday: ['誕生日', 'たんじょうび'],
  gift: ['プレゼント', '贈り物', 'ギフト'],
  christmas: ['クリスマス'],
  halloween: ['ハロウィン'],

  // Objects
  phone: ['電話', 'でんわ', 'スマホ'],
  computer: ['パソコン', 'コンピュータ'],
  laptop: ['ノートパソコン', 'ラップトップ'],
  camera: ['カメラ'],
  tv: ['テレビ'],
  money: ['お金', 'おかね', '金'],
  key: ['鍵', 'かぎ', 'キー'],
  lock: ['錠', 'ロック'],
  tool: ['工具', 'ツール', '道具'],
  book: ['本', 'ほん', '書籍'],
  email: ['メール', 'Eメール'],
  trash: ['ゴミ', 'ごみ', 'ゴミ箱'],
  pill: ['薬', '錠剤', 'くすり'],
  bomb: ['爆弾', 'ばくだん', 'ボム'],
  crown: ['王冠', 'おうかん', 'クラウン'],
  ring: ['指輪', 'ゆびわ', 'リング'],
  glasses: ['眼鏡', 'メガネ', 'めがね'],
  shoe: ['靴', 'くつ', 'シューズ'],
  hat: ['帽子', 'ぼうし'],
  scissors: ['はさみ', 'ハサミ'],
  pencil: ['鉛筆', 'えんぴつ'],
  pen: ['ペン', 'ボールペン'],
  folder: ['フォルダ', 'ファイル'],
  package: ['荷物', 'パッケージ', '小包'],
  cart: ['カート', '買い物'],
  shield: ['盾', 'シールド', 'たて'],
  sword: ['剣', 'つるぎ', 'ソード'],
  swords: ['剣', '刀'],
  teddy: ['テディベア', 'ぬいぐるみ'],
  dress: ['ドレス', 'ワンピース'],
  backpack: ['リュック', 'バックパック'],
  lipstick: ['口紅', 'リップ', 'くちべに'],
  science: ['科学', 'かがく', 'サイエンス'],
  dna: ['DNA', '遺伝子'],

  // More food
  soup: ['スープ', '汁物'],
  salad: ['サラダ'],
  noodles: ['麺', 'ラーメン', 'めん'],
  shrimp: ['エビ', 'えび'],
  donut: ['ドーナツ'],
  popcorn: ['ポップコーン'],
  butter: ['バター'],
  salt: ['塩', 'しお'],
  spicy: ['辛い', 'からい', 'スパイシー'],
  sweet: ['甘い', 'あまい', 'スイート'],
  delicious: ['美味しい', 'おいしい'],
  hungry: ['お腹すいた', '空腹', 'はらぺこ'],
  drink: ['飲み物', '飲む', 'ドリンク'],
  cocktail: ['カクテル'],
  champagne: ['シャンパン'],
  juice: ['ジュース'],

  // More animals
  elephant: ['象', 'ゾウ', 'ぞう'],
  giraffe: ['キリン', 'きりん'],
  zebra: ['シマウマ', 'しまうま'],
  gorilla: ['ゴリラ'],
  camel: ['ラクダ', 'らくだ'],
  kangaroo: ['カンガルー'],
  deer: ['鹿', 'しか'],
  owl: ['フクロウ', 'ふくろう'],
  eagle: ['鷲', 'ワシ', 'イーグル'],
  duck: ['アヒル', 'あひる', 'カモ'],
  penguin: ['ペンギン'],
  parrot: ['オウム', 'おうむ', 'インコ'],
  flamingo: ['フラミンゴ'],
  swan: ['白鳥', 'はくちょう', 'スワン'],
  wolf: ['狼', 'オオカミ', 'おおかみ'],
  bat: ['コウモリ', 'こうもり'],
  scorpion: ['サソリ', 'さそり'],
  snail: ['カタツムリ', 'かたつむり'],
  ant: ['蟻', 'アリ', 'あり'],
  mosquito: ['蚊', 'か'],
  worm: ['ミミズ', '虫', 'みみず'],
  octopus: ['タコ', 'たこ'],
  squid: ['イカ', 'いか'],
  crocodile: ['ワニ', 'わに'],
  lizard: ['トカゲ', 'とかげ'],
  hedgehog: ['ハリネズミ'],
  otter: ['カワウソ', 'かわうそ'],
  sloth: ['ナマケモノ', 'なまけもの'],
  beaver: ['ビーバー'],
  llama: ['ラマ', 'リャマ'],
  goat: ['ヤギ', 'やぎ'],
  sheep: ['羊', 'ひつじ'],
  hippo: ['カバ', 'かば'],
  rhino: ['サイ', 'さい'],
  cactus: ['サボテン', 'さぼてん'],
  clover: ['クローバー', '四つ葉'],
  leaf: ['葉', 'はっぱ', '葉っぱ'],
  plant: ['植物', 'しょくぶつ'],
  herb: ['ハーブ', '薬草'],

  // More travel
  volcano: ['火山', 'かざん'],
  globe: ['地球', 'ちきゅう', 'グローブ'],
  weather: ['天気', 'てんき'],
  storm: ['嵐', 'あらし'],
  thunder: ['雷', 'かみなり'],
  wind: ['風', 'かぜ'],
  cold: ['寒い', 'さむい', '冷たい'],
  hot: ['暑い', 'あつい', '熱い'],
  cloud: ['雲', 'くも'],

  // More activities
  boxing: ['ボクシング'],
  karate: ['空手', 'からて'],
  archery: ['弓道', 'アーチェリー'],
  fishing: ['釣り', 'つり'],
  bowling: ['ボウリング'],
  chess: ['チェス', '将棋'],
  dice: ['サイコロ', 'ダイス'],
  target: ['的', 'まと', 'ターゲット'],
  dart: ['ダーツ'],
  casino: ['カジノ'],
  fireworks: ['花火', 'はなび'],
  balloon: ['風船', 'バルーン', 'ふうせん'],

  // More people
  child: ['子供', 'こども'],
  adult: ['大人', 'おとな'],
  elder: ['高齢者', 'お年寄り'],
  beard: ['ひげ', '髭'],
  blonde: ['金髪', 'ブロンド', 'きんぱつ'],
  guard: ['警備員', 'ガード'],
  construction: ['工事', '建設'],
  royal: ['王族', 'ロイヤル'],
  king: ['王', '王様', 'おうさま'],
  queen: ['女王', 'クイーン', 'じょおう'],
  mage: ['魔法使い', '魔導士'],
  wizard: ['魔法使い', 'ウィザード'],
  fairy: ['妖精', 'ようせい', 'フェアリー'],
  mermaid: ['人魚', 'にんぎょ', 'マーメイド'],
  vampire: ['吸血鬼', 'ヴァンパイア'],
  magic: ['魔法', 'まほう', 'マジック'],
  deaf: ['聴覚障害', '耳が聞こえない'],
  bow: ['お辞儀', 'おじぎ', '弓'],

  // Symbols
  check: ['チェック', '確認'],
  no: ['禁止', 'いいえ', 'ダメ'],
  warning: ['警告', '注意', 'けいこく'],
  question: ['疑問', 'はてな', '質問'],
  exclamation: ['感嘆符', 'びっくり'],
  infinity: ['無限', 'むげん'],
  recycle: ['リサイクル', '再利用'],
  play: ['再生', 'プレイ'],
  pause: ['一時停止', 'ポーズ'],
  shuffle: ['シャッフル', 'ランダム'],
  repeat: ['繰り返し', 'リピート'],
  plus: ['プラス', '足す'],
  minus: ['マイナス', '引く'],
  divide: ['割る', '除算'],
  multiply: ['掛ける', '乗算'],
  copyright: ['著作権', 'コピーライト'],
  trademark: ['商標', 'トレードマーク'],
  note: ['メモ', 'ノート', '音符'],
  notes: ['音符', 'メモ'],
  red: ['赤', 'あか', 'レッド'],
  blue: ['青', 'あお', 'ブルー'],
  green: ['緑', 'みどり', 'グリーン'],
  yellow: ['黄色', 'きいろ', 'イエロー'],
  purple: ['紫', 'むらさき', 'パープル'],
  black: ['黒', 'くろ', 'ブラック'],
  white: ['白', 'しろ', 'ホワイト'],
  brown: ['茶色', 'ちゃいろ', 'ブラウン'],
  pink: ['ピンク', '桃色'],
  grey: ['灰色', 'グレー', 'はいいろ'],

  // Professions
  doctor: ['医者', 'いしゃ', '医師'],
  teacher: ['先生', 'せんせい', '教師'],
  student: ['学生', 'がくせい', '生徒'],
  judge: ['裁判官', '判事'],
  farmer: ['農家', 'のうか', '農民'],
  chef: ['シェフ', '料理人'],
  cook: ['料理', '調理', 'コック'],
  mechanic: ['整備士', 'メカニック'],
  factory: ['工場', 'こうじょう'],
  worker: ['労働者', '作業員', 'ワーカー'],
  office: ['オフィス', '事務所', '会社'],
  scientist: ['科学者', 'かがくしゃ'],
  programmer: ['プログラマー'],
  developer: ['開発者', 'デベロッパー'],
  coder: ['コーダー', 'プログラマー'],
  singer: ['歌手', 'シンガー', 'かしゅ'],
  artist: ['アーティスト', '芸術家'],
  painter: ['画家', 'ペインター'],
  pilot: ['パイロット', '操縦士'],
  astronaut: ['宇宙飛行士', 'うちゅうひこうし'],
  firefighter: ['消防士', 'しょうぼうし'],

  // Family
  family: ['家族', 'かぞく', 'ファミリー'],
  parents: ['両親', 'りょうしん', '親'],
  couple: ['カップル', '夫婦'],
  son: ['息子', 'むすこ'],
  daughter: ['娘', 'むすめ'],
  children: ['子供たち', 'こどもたち'],
  pregnant: ['妊娠', 'にんしん', '妊婦'],

  // Person actions
  walk: ['歩く', 'あるく', '散歩'],
  jog: ['ジョギング', '軽く走る'],
  kneel: ['ひざまずく', '膝をつく'],
  stand: ['立つ', 'たつ'],

  // More animals 2
  jellyfish: ['クラゲ', 'くらげ'],
  seal: ['アザラシ', 'あざらし'],
  coral: ['珊瑚', 'サンゴ', 'さんご'],
  beetle: ['カブトムシ', '甲虫'],
  cockroach: ['ゴキブリ', 'ごきぶり'],
  cricket: ['コオロギ', 'こおろぎ'],
  dove: ['鳩', 'ハト', 'はと'],
  skunk: ['スカンク'],
  badger: ['アナグマ', 'あなぐま'],
  mammoth: ['マンモス'],
  bison: ['バイソン', '野牛'],
  feather: ['羽', 'はね', '羽根'],
  paw: ['肉球', 'にくきゅう'],
  palm: ['ヤシ', 'やし', '手のひら'],
  seedling: ['芽', '苗', 'め'],
  lotus: ['蓮', 'ハス', 'はす'],
  tulip: ['チューリップ'],
  bouquet: ['花束', 'はなたば', 'ブーケ'],

  // More food 2
  waffle: ['ワッフル'],
  pretzel: ['プレッツェル'],
  bagel: ['ベーグル'],
  dumpling: ['餃子', 'ぎょうざ', '団子'],
  mate: ['マテ茶'],
  sake: ['お酒', '日本酒', 'さけ'],
  ginger: ['生姜', 'しょうが', 'ジンジャー'],
  olive: ['オリーブ'],
  beans: ['豆', 'まめ'],

  // Signs
  atm: ['ATM', '現金自動預払機'],
  restroom: ['トイレ', 'お手洗い', '化粧室'],
  wheelchair: ['車椅子', 'くるまいす'],
  parking: ['駐車場', 'ちゅうしゃじょう', 'パーキング'],
  peace: ['平和', 'へいわ', 'ピース'],

};
